...
var statefulsetCmd = &cobra.Command{
	...
	Aliases:           []string{"sts"},
	Short:             "Show resource tree for a StatefulSet",
	Args:              cobra.ArbitraryArgs,
	ValidArgsFunction: completeResourceNames(listStatefulSetNames),
	RunE: func(cmd *cobra.Command, args []string) error {
		args, err := resolveArgs(args, "statefulset", listStatefulSetNames)
		if err != nil {
			return err
		}
		...
		return runWatch(func(w io.Writer) error {
			for i, name := range args {
				...
				root, err := builder.BuildStatefulSetTree(context.Background(), namespace, name)
				if err != nil {
					return fmt.Errorf("building statefulset tree: %w", err)
				}
